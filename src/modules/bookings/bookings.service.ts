...
// creating a booking
const createBooking = async (payload: Record<string, any>, user: { id: number; role: string; }) => {

  const { vehicle_id, rent_start_date, rent_end_date, customer_id } = payload;

  if (user.role !== "admin" && user.id !== customer_id) {
    throw new ApiError(403, "Forbidden, please try enter your own valid id");
  }
  ...
  if (totalDays <= 0) {
    throw new ApiError(400, "Invalid rental date range");
  }
...
const getAllBookings = async (user: { id: number; role: string }) => {
  ...
  const { rows: bookings } = await pool.query(query, values);
...
//update bookings
const adminUpdateBooking = async (bookingId: string, payload: { status?: string; }) => {

  if (payload.status !== "returned") {
    throw new ApiError(400, "Admin can only set booking status to returned");
  }
  ...
  try {
    ...
    if (bookingRes.rows[0].status !== "active") {
      throw new ApiError(400, "Only active bookings can be returned");
    }
...
const cancelBookingByOwner = async (
  bookingId: string,
  userId: number,
  ...
  if (payload.status !== "cancelled") {
    throw new ApiError(
      400,
      "Invalid request body. You can only cancel the booking."
    );
  }
  ...
  try {
    ...
    if (
      ...
      throw new ApiError(
        400,
        "Only active bookings that have not started can be cancelled"
      );
    }
